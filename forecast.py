...
def generate_ai_recommendation(kpi_df, detailed_df, api_key):
    """Generate AI-powered recommendation using Claude via OpenRouter"""
    
    # Use provided key or get from environment - for our claude api - used OpenRouter - awesome place to use different models for calibrated tasks
    if not api_key:
        api_key = os.getenv('OPENROUTER_API_KEY')
    
    if not api_key:
        return " API key not found!! Please set OPENROUTER_API_KEY environment variable."
...
## YOUR TASK:
Write a SIMPLE recommendation (500-600) words maximum) that:
...
3. Talk about comparison and effect on lag, that is, more lag, less data recency, on the KPIs.
4. Gives 3-4 clear action steps
5. Explain technical jargons like, what a KPI used in answer means, and give examples wherever necessary.

Use everyday language. Avoid jargon. Pretend you're explaining to someone who is demand planner and what they will like to see. Use short sentences and bullet points with evident examples wherever necessary. 
"""
...
def plot_complete_history_with_forecast(monthly_df, detailed_df, sku_id, lag_period, model_name):
    ...
    # Add vertical line at forecast start - better than using fig.add_vline function as it doesn't suite when we combine forecasts
    # and historical sales in one graph
    fig.add_shape(
        type="line",
        x0=forecast_start,
        x1=forecast_start,
    ...
    # Calculate MAE for title header in SKU trend subpage
...
def main():
    ...
    # Model selection - ARIMA as default - as its best given the simplicity of our data. Prophet is also great.
    st.sidebar.subheader("Model Selection")
    all_models = sorted(kpi_df['Model'].unique().tolist())
    
    # Set ARIMA as default
    default_model_idx = 0
    ...
    # figured one SKU level analysis is great, given only 10 SKUs, combining them would be overlloking our KPIs
    ...
    #  MAIN CONTENT 
    st.title("Sales Forecast Lag Analysis Dashboard")
    st.markdown("### Interactive Analysis of Forecast Performance Across Different Update Frequencies across KPIs")
    st.markdown("---")
    ...
    with tab1:
        ...
        # Complete Historical View with Forecast Overlay Graph
        ...
        # Detailed Forecast View - Zoomed IN
    ...
    with tab5:
        ...
        has_api_key = os.getenv('OPENROUTER_API_KEY') is not None
        ...
        if has_api_key:
            ...
            st.warning("Error -  OPENROUTER_API_KEY not set in environment variables")
            
            col_ai1, col_ai2 = st.columns([2, 2])
            
            with col_ai1:
                api_key = st.text_input(
                    "OpenRouter API Key (Optional)",
                    type="password",
                    help="Enter your OpenRouter API key manually",
                    key='api_key_rec'
                )
        ...
        if generate_rec:
            with st.spinner("Generating AI-powered insights..."):
                manual_key = api_key if 'api_key' in locals() else None
                recommendation = generate_ai_recommendation(kpi_filtered, detailed_filtered, manual_key)
        ...
        # General best practices - overall for our data
...
# RUN APP - Let's fullfill the demands
